...
class NaiveBayesPredictor(Predictor):
    def __init__(self, model: GaussianNB, team_encoding: OneHotEncoder) -> None:
        self.model = model
    ...
    def predict(self, fixture: Fixture) -> Prediction:
        ...
        x: NDArray[float64] = np.concatenate(
            [encoded_home_name, encoded_away_name], 1)
...
def build_model(results: List[Result]) -> Tuple[GaussianNB, OneHotEncoder]:
    ...
    x: NDArray[float64] = np.concatenate(
        [encoded_home_names, encoded_away_names], 1)
...
def train_naive_bayes_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)

    return NaiveBayesPredictor(model, team_encoding)
